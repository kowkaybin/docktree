 ...
 * Description: Lightweight DOM-first layout engine with side-by-side isolated preview, interactive panel scaling, and shortcode parsing.
 * Version: 1.1.0
...
function docktree_admin_assets($hook) {
    ...
    if (in_array($hook, array('post.php', 'post-new.php')) && $post_type === 'page') {
        wp_enqueue_style('docktree-admin-css', DOCKTREE_URL . 'assets/css/admin-style.css', array(), '1.1.0');
        wp_enqueue_script('docktree-admin-js', DOCKTREE_URL . 'assets/js/editor.js', array('jquery'), '1.1.0', true);
